		...
		<div className="w-full flex flex-col h-[100vh]">
			...
			<div className="w-[90%] pt-5 self-center">
					...
					<div className="flex items-center gap-x-10">
						<div className="flex items-center gap-x-2">
							<div
								...
								className={`px-3 py-1 rounded-sm text-sm hover:cursor-pointer ${showActive ? "bg-green-600 text-white" : ""}`}
							>
								Active
							</div>
							<div
				...
				<ScrollArea className="h-screen">
					<Table sx={{ minWidth: 800 }} verticalSpacing="sm" horizontalSpacing="lg">
						<thead>
							<tr>
								<th>First Name</th>
								<th>Last Name</th>
						...
						{showActive
							? employees?.map((el) => {
									if (el.firstName.toLowerCase().includes(search.toLowerCase()) && el.active) {
										return (
											<EmployeeCRUD
												key={el.id}
												email={el.email}
